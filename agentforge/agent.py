...
class CodexCLI:
    ...
    def run(prompt: str, cwd: Path, timeout: int, env: dict,
            output_name: str = "agent_output.json",
    ...
    def _build_implement_prompt(
        ...
        return (
            ...
            f"FILE WRITING RULES:\n"
            f"  - When writing Python files, use regular quotes, NOT escaped quotes\n"
            f"  - WRONG: modules = [\\\"torch\\\", \\\"numpy\\\"]\n"
            f"  - RIGHT: modules = [\"torch\", \"numpy\"]\n"
            f"  - Use cat with heredoc (<<'EOF') to write files, not echo\n\n"
            f"OUTPUT (CRITICAL — you MUST do this as your FINAL step):\n"
            f"  mkdir -p .agentforge\n"
    ...
    @staticmethod
    def _fix_escaped_quotes(cwd: Path) -> None:
        """Fix escaped quotes in Python files generated by Codex.

        Codex sometimes writes \\\" instead of " when generating Python files
        through shell commands, causing SyntaxError.
        """
        for py_file in Path(cwd).rglob("*.py"):
            try:
                content = py_file.read_text()
                if '\\"' in content:
                    py_file.write_text(content.replace('\\"', '"'))
    ...
    def implement_strategy(
        ...
        # Post-fix: clean escaped quotes in generated Python files
        CodexCLI._fix_escaped_quotes(cwd)
